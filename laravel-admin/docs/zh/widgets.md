...
`Omaicode\Admin\Widgets\Box`用来生成box组件：
...
`Omaicode\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`Omaicode\Admin\Widgets\Form`类用来快速构建表单：
...
`Omaicode\Admin\Widgets\InfoBox`类用来生成信息展示块：
...
`Omaicode\Admin\Widgets\Tab`类用来生成选项卡组件：
...
`Omaicode\Admin\Widgets\Table`类用来生成表格：
