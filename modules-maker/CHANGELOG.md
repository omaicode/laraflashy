...
- Fixed bug: `Target class [Omaicode\Modules\Commands\] does not exist.`
...
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Omaicode\Modules\Module`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Omaicode\Modules\Module`.
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Omaicode\Modules\FileRepository`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Omaicode\Modules\FileRepository`.
- **BREAKING** Removed the `__get` magic method on the `\Omaicode\Modules\Module` class. Use `get()` or `json()->get()` instead.
